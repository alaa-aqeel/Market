...
class ItemUserController extends Controller
{
     ...
     * @return void
     */
    function __construct(ItemInterface $itemInterface)
    {
        $this->middleware(function(Request $request, \Closure $next) use($itemInterface){
            $this->item = $itemInterface->init(auth()->user()->items());
            return $next($request);
        });
    }
